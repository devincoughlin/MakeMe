...
@class BBEditPrintSettings, BBEditItem, BBEditApplication, BBEditClipboard, BBEditDocument, BBEditFactoryDocument, BBEditFile, BBEditFilter, BBEditFilter_term, BBEditProjectItem, BBEditProjectCollection, BBEditProjectDocument, BBEditQuickTimeDocument, BBEditShellDocument, BBEditTextDocument, BBEditWebSite, BBEditWindow, BBEditAbout_window, BBEditDifferences_window, BBEditDiskBrowserWindow, BBEditFactoryWindow, BBEditFind_window, BBEditMultifile_search_window, BBEditFTPBrowserWindow, BBEditOpen_file_by_name_window, BBEditPalette, BBEditMultifile_differences_window, BBEditPlugin_info_window, BBEditPreferences_window, BBEditQuickTimeWindow, BBEditResultsBrowser, BBEditSearchResultsBrowser, BBEditShellWindow, BBEditTextWindow, BBEditClipboard_window, BBEditScratchpad_window, BBEditProjectWindow, BBEditPerl_reference_window, BBEditSource_control_form_window, BBEditToolWindow, BBEditWeb_preview_window, BBEditCompareOptions, BBEditCompareResults, BBEditFTPInfo, BBEditMarkOptions, BBEditResultEntry, BBEditSearchOptions, BBEditSearchMatch, BBEditSearchStringProperties, BBEditSortLinesOptions, BBEditSortLinesOutputOptions, BBEditProcessDuplicatesOptions, BBEditProcessDuplicatesOutputOptions, BBEditProcessDuplicatesResults, BBEditProcessLinesContainingOutputOptions, BBEditProcessLinesContainingResults, BBEditText_object, BBEditInsertionPoint, BBEditCharacter, BBEditWord, BBEditLine, BBEditDisplay_line, BBEditText, BBEditHTMLPrefs, BBEditTagInfo, BBEditTagResult, BBEditTagPairResult;
...
enum BBEditOMd {
	BBEditOMdUnixScriptOutputWindow = 'mprl',
	BBEditOMdNewUntitledWindow = 'nwin',
	BBEditOMdScriptLogOutputWindow = 'lwin'
};
typedef enum BBEditOMd BBEditOMd;

enum BBEditFOM {
	BBEditFOMUnixScriptOutputWindow = 'mprl',
	BBEditFOMNewUntitledWindow = 'nwin'
};
typedef enum BBEditFOM BBEditFOM;
...
@interface BBEditPrintSettings : SBObject
...
- (void) copyAppendingToClipboard:(BOOL)appendingToClipboard;  // Copy an object to the clipboard
...
- (NSString *) zapGremlinsZapAction:(BBEditZAct)zapAction zapCharacter:(NSString *)zapCharacter nulls:(BOOL)nulls controls:(BOOL)controls nonASCIICharacters:(BOOL)nonASCIICharacters;  // Removes or replaces unwanted characters from text
...
- (SBElementArray *) toolWindows;
- (SBElementArray *) clipboards;
- (SBElementArray *) webSites;
...
@property (copy, readonly) BBEditAbout_window *aboutBox;  // the about box
@property (copy) id currentClipboard;  // the clipboard
@property (copy) NSString *currentClippingSet;  // the name of the active clipping set
...
@property (copy, readonly) BBEditClipboard_window *clipboardWindow;  // the “Clipboard” window
@property (copy) BBEditScratchpad_window *scratchpadWindow;  // the “Scratchpad” window
@property (copy) BBEditShellWindow *UnixWorksheetWindow;  // the “Unix Worksheet” window
...
@property (copy, readonly) BBEditPalette *ASCIITable;  // the floating window displaying ASCII character and escape sequences
...
@property (copy, readonly) BBEditPalette *textFactoriesList;  // the floating window displaying available text factories
@property (copy, readonly) BBEditPalette *stationeryList;  // the floating window displaying available stationery items
...
@property (copy, readonly) BBEditPalette *CSSPalette;  // the main HTML tools palette
@property (copy, readonly) BBEditPalette *HTMLBlockPalette;  // the HTML block elements palette
@property (copy, readonly) BBEditPalette *HTMLInlinePalette;  // the HTML inline elements palette
@property (copy, readonly) BBEditPalette *HTMLTablePalette;  // the HTML table elements palette
@property (copy, readonly) BBEditPalette *HTMLFormPalette;  // the HTML form elements palette
@property (copy, readonly) BBEditPalette *HTMLPhrasePalette;  // the HTML phrase elements palette
@property (copy, readonly) BBEditPalette *HTMLFontPalette;  // the HTML font elements palette
...
@property (copy) BBEditWebSite *defaultWebSite;  // the current default entry in the HTML Web Site preferences
...
- (NSInteger) replace:(NSString *)x using:(NSString *)using_ searchingIn:(id)searchingIn searchingString:(NSString *)searchingString options:(BBEditSearchOptions *)options saving:(BBEditSavo)saving recursion:(BOOL)recursion textFilesOnly:(BOOL)textFilesOnly skipShieldedFolders:(BOOL)skipShieldedFolders;  // performs a Replace All
...
- (BBEditHTMLPrefs *) getHtmlPreferences;  // gets the current html preferences (the accessors for the web site preferences are maintained for backwards compatability, but new scripts should use the web site element(s) of the application)
...
- (void) runUnixScript:(NSArray *)x selectionOnly:(BOOL)selectionOnly saveBeforeRunning:(BOOL)saveBeforeRunning useDebugger:(BOOL)useDebugger runInTerminal:(BOOL)runInTerminal chdirToScriptFolder:(BOOL)chdirToScriptFolder outputTo:(BBEditOMd)outputTo clearBeforeWriting:(BOOL)clearBeforeWriting saveAfterWriting:(BOOL)saveAfterWriting;  // runs the unix script in the front BBEdit window
- (void) runUnixFilter:(NSURL *)x outputSTDERRTo:(BBEditFOM)outputSTDERRTo clearBeforeWriting:(BOOL)clearBeforeWriting saveAfterWriting:(BOOL)saveAfterWriting replacingSelection:(BOOL)replacingSelection;  // runs the specified unix filter on the selection in the frontmost BBEdit window

@end

// A clipboard
@interface BBEditClipboard : SBObject
...
- (void) runUnixFilterOutputSTDERRTo:(BBEditFOM)outputSTDERRTo clearBeforeWriting:(BOOL)clearBeforeWriting saveAfterWriting:(BOOL)saveAfterWriting replacingSelection:(BOOL)replacingSelection;  // runs the specified unix filter on the selection in the frontmost BBEdit window

@end

// a file filter
@interface BBEditFilter : SBObject
...
@interface BBEditFilter_term : SBObject
...
- (SBElementArray *) projectItems;
- (SBElementArray *) projectCollections;
...
// a document opened from a QuickTime-translated image file
@interface BBEditQuickTimeDocument : BBEditDocument
...
@property (copy, readonly) BBEditWebSite *containerWebSite;  // if contained in a defined web site, a reference to that web site entry else missing value
...
// an entry in the HTML Web Site preferences
@interface BBEditWebSite : BBEditItem

@property (readonly) NSInteger ID;  // object’s unique id
@property (copy) NSString *name;  // the name
@property (copy) NSString *serverUrl;  // the server url, for example http://www.barebones.com/
@property (copy) NSString *serverPath;  // the server path, for example bbedit/
@property (copy) NSString *defaultFileName;  // the default file used by the link checker, for example index.html
@property (copy) NSURL *rootDirectory;  // the folder in which your html files reside
@property (copy) NSURL *templateDirectory;  // the folder in which your templates reside
@property (readonly) BOOL isDefaultWebSite;  // is this the current default site in the HTML Web Site preferences
@property (readonly) BOOL useLocalPreviewServer;  // preview through local web server instead of filesystem?
@property (copy) NSString *previewServerUrl;  // the preview server url, for example http://localhost/~username/testsite/
...
// an about box window
@interface BBEditAbout_window : BBEditWindow


@end

// a single-file Differences window
@interface BBEditDifferences_window : BBEditWindow
...
@interface BBEditMultifile_differences_window : BBEditDifferences_window
...
// a window displaying the contents of a QuickTime-translated image file
@interface BBEditQuickTimeWindow : BBEditWindow
...
@interface BBEditShellWindow : BBEditWindow
...
// A clipboard window
@interface BBEditClipboard_window : BBEditTextWindow


@end

// A scratchpad window
@interface BBEditScratchpad_window : BBEditTextWindow
...
// A palette created by a BBEdit plug-in
@interface BBEditToolWindow : BBEditPalette
...
@interface BBEditCompareOptions : SBObject
...
@interface BBEditCompareResults : SBObject
...
@interface BBEditFTPInfo : SBObject
...
@interface BBEditMarkOptions : SBObject
...
@interface BBEditResultEntry : SBObject
...
@interface BBEditSearchOptions : SBObject
...
@interface BBEditSearchMatch : SBObject
...
@interface BBEditSearchStringProperties : SBObject

@property (copy, readonly) NSString *searchString;  // search text
@property (copy, readonly) NSString *replacementString;  // replace text
@property (readonly) NSInteger searchStringScript;  // script language code of the search text
@property (readonly) NSInteger replaceStringScript;  // script language code of the search text
...
@interface BBEditSortLinesOptions : SBObject
...
@interface BBEditSortLinesOutputOptions : SBObject
...
@interface BBEditProcessDuplicatesOptions : SBObject
...
@interface BBEditProcessDuplicatesOutputOptions : SBObject
...
@interface BBEditProcessDuplicatesResults : SBObject
...
@interface BBEditProcessLinesContainingOutputOptions : SBObject
...
@interface BBEditProcessLinesContainingResults : SBObject
...
- (NSInteger) replaceUsing:(NSString *)using_ searchingIn:(id)searchingIn searchingString:(NSString *)searchingString options:(BBEditSearchOptions *)options saving:(BBEditSavo)saving recursion:(BOOL)recursion textFilesOnly:(BOOL)textFilesOnly skipShieldedFolders:(BOOL)skipShieldedFolders;  // performs a Replace All
...
@interface BBEditHTMLPrefs : SBObject
...
@property (copy) NSString *serverUrl;  // the server url, for example http://www.barebones.com/ (deprecated - use the new “web site” element of the application)
@property (copy) NSString *serverPath;  // the server path, for example bbedit/ (deprecated - use the new “web site” element of the application)
@property (copy) NSString *defaultFileName;  // the default file used by the link checker, for example index.html (deprecated - use the new “web site” element of the application)
@property (copy) NSURL *templateDirectory;  // the folder in which your templates reside (deprecated - use the new “web site” element of the application)
@property (copy) NSURL *rootDirectory;  // the folder in which your html files reside (deprecated - use the new “web site” element of the application)
@property BBEditPRVW previewMode;  // the mode used to preview unsaved files
...
// tag info record
@interface BBEditTagInfo : SBObject
...
@interface BBEditTagResult : SBObject
...
@interface BBEditTagPairResult : SBObject
